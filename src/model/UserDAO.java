...
public class UserDAO {
	...
	public static ArrayList<UserDTO> loginValidate(UserDTO user) throws SQLException {
		...
		try {
			...
			pstmt = con.prepareStatement("select emailAddress, password, name, sectorOne, sectorTwo, sectorThree from person where emailAddress=?");// 요렇게
	...
	public static boolean addUser(UserDTO user) throws SQLException {
		...
		try {
			con = DBUtil.getConnection();
			pstmt = con.prepareStatement(sql.getString("addPerson"));//요렇게
