...
def getStock(symbol) :
    ...
    param['x'] = exchange_info.loc[exchange_info['Exchange'] == str(exchange),'Google']
    print(param['x'])
    param['c'] = exchange_info.loc[exchange_info['Exchange'] == str(param['x']),'Currency']
    
    
    lists_4 = ['HKG','TPE','TYO','SHE','TPEX']
    lists_6 = ['KRX','KOSDAQ','SHA']
    ...
    for d in data.values:
      ...
      #  print(exchange)
        ...
        if not stock.empty:
            ...
            print(param['q'] + "." + param['x'] + " success")
        else:
            print(param['q'] + "." + param['x'] + " fail")
    print(stocks)
